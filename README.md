...
このままでは Genesis ブロックで割り当てた残高の移動ができない。
試しに既知のノード・既知のアカウントからのリクエストであることを検証するようにしてみる。

`ApproveTx()` が受けるオブジェクトは変更前のスクリプトで確認しておく。
...
ルールを変更した場合は、`clef attest` の差異実施が必要であることに注意。

なお、ここまでで構成したプライベートネットワークでは、
ノードとアカウントの組み合わせが異なる場合、
そもそも Clef にパスワードが保存されていないため、
トランザクションは受け入れられない。
